...
Reviewed locally on September 12, 2026. These checks cover the original
`companion/` extension. The course letter has its own
...
| --- | --- |
| HTML | W3C Nu checker 26.9.9 returned no messages for the initial companion. A final external recheck after changing the Project notes link is pending. |
...
5. Check actual 200% browser zoom and listen to the page with VoiceOver. These
   ...
   do not replace them.
6. Recheck the final HTML with the [W3C Nu checker](https://validator.w3.org/nu/).
   The final external submission was not run. Local path and structure checks
   do not establish full HTML conformance.
7. After publication, verify the companion URL, CSS response, navigation back
   to the course letter, and the preview image. The companion is not yet live.
